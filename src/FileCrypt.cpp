...
FileCrypt::FileCrypt(std::string keys_path) {
    this->load_keys(keys_path);
}
...
std::string* FileCrypt::encrypt_internal(std::fstream *file, std::string &buffer) {
...
std::string* FileCrypt::decrypt_internal(std::fstream *file, std::string &buffer) {
...
void FileCrypt::crypt(std::string path, FileCrypt::Operation op) {
    if(boost::filesystem::is_directory(path) == true) {
        ...
        for (boost::filesystem::recursive_directory_iterator it(path.c_str()); it != end1; ++it) {
...
void FileCrypt::file(std::string path, FileCrypt::Operation op) {
    std::fstream *file = this->open_file(path, std::fstream::in | std::fstream::binary);
...
void FileCrypt::dir(std::string path, FileCrypt::Operation op) {
    ...
    for (boost::filesystem::recursive_directory_iterator it(path.c_str()); it != end2; ++it) {
        boost::filesystem::path boost_path = it->path();
        if(boost::filesystem::is_directory(boost_path)) {
            continue;
        }
